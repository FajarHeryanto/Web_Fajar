...
(function(window) {
  ...
  function computeFrame(sdfFn, cols, rows, rotYA, rotXA, viewSize){
    ...
    // OPTIMIZATION: Drastically reduced steps for better FPS
    ...
    for(let y=0;y<rows;y++){
      for(let x=0;x<cols;x++){
        ...
        // OPTIMIZATION: exit early at t>14 (saves iterations on background rays)
        for(let i=0;i<steps;i++){hp=v3add(ro,v3scale(rd,t));const dd=sdfFn(hp);if(dd<0.05){hit=true;break;}t+=dd;if(t>14)break;}
        const idx=y*cols+x;
        if(hit){
  ...
  class AsciiBackground {
    constructor(canvas,cfg={}){
      ...
      // OPTIMIZATION: Increased fontSize to reduce raymarching grid size (better FPS)
      this.cfg={fontSize:isMobile?16:(cfg.fontSize||16),maxFps:isMobile?10:(cfg.maxFps||15),...cfg};
  ...
  class Dragon3D {
    constructor(canvas,cfg={}){
      this.canvas=canvas;this.ctx=canvas.getContext('2d');
      // OPTIMIZATION: Increased fontSize to reduce raymarching grid size (better FPS)
      this.cfg={fontSize:isMobile?14:(cfg.fontSize||15),maxFps:isMobile?10:(cfg.maxFps||15),...cfg};
      this.running=false;this.lastFrame=0;this.frameInterval=1000/this.cfg.maxFps;
