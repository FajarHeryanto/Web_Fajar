document.addEventListener('DOMContentLoaded', function() {
  ...
  function setNav(open) {
    navOpen = open;
    if (navDropdown) navDropdown.classList.toggle('open', open);
    if (menuToggle) {
      const lbl = menuToggle.querySelector('.label');
      if (lbl) lbl.textContent = open ? 'Close' : 'Menu';
    }
  }
  if (menuToggle) menuToggle.addEventListener('click', () => setNav(!navOpen));
  document.addEventListener('click', e => { if (navOpen && !e.target.closest('.center-nav')) setNav(false); });
  ...
  navItems.forEach(item => {
    item.addEventListener('click', () => {
      ...
      if (target) {
        ...
        if (sec) { setNav(false); setTimeout(() => sec.scrollIntoView({ behavior: 'smooth' }), 250); }
      }
    });
  });
  ...
  function setTheme(t) {
    ...
    document.querySelectorAll('.theme-dot,.center-dot[data-theme]').forEach(d => {
  ...
  function initAllAscii() {
    ...
    if (!navDragonInited) { const nc = document.getElementById('nav-dragon-canvas'); if (nc && window.AsciiImage) { navDragonInited = true; new AsciiImage(nc, { fontSize: 7, style: 'geometric', seed: 42 }).setupCanvas(); } }
  }
